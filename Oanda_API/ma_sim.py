...
def evaluate_pair(i_pair, mashort, malong, price_data):
    price_data = price_data[['time', 'bid_c', get_ma_col(mashort), get_ma_col(malong)]].copy()
...
def get_price_data(pairname, granularity):
    ...
    return df[['time', 'bid_c']]


def process_data(ma_short, ma_long, price_data):
    ma_list = set(ma_short + ma_long)
    for ma in ma_list:
        price_data[get_ma_col(ma)] = price_data.bid_c.rolling(window=ma).mean()
    return price_data
...
def get_test_pairs(pair_str):
    existing_pairs = instruments.Instrument.get_instruments_dict().keys()
...
def run():
    ...
    for pairname in test_pairs:
        ...
        i_pair = instruments.Instrument.get_instruments_dict()[pairname]
